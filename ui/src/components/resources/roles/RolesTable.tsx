...
export default function RolesTable({ token, namespace }: { token: string; namespace: string }) {
  ...
  const fetchRows = useCallback(async () => {
    const res = await apiGet<any>(`/api/namespaces/${encodeURIComponent(namespace)}/roles`, token);
  ...
  const accessDenied = useEmptyListAccessCheck({
    ...
    error,
    loading,
    resource: listResourceAccess.roles,
    namespace,
  });
