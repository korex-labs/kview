...
export default function ClusterRolesTable({ token }: { token: string }) {
  ...
  const fetchRows = useCallback(async () => {
    const res = await apiGet<any>("/api/clusterroles", token);
    const items: ClusterRole[] = res.items || [];
    return items.map((role) => ({ ...role, id: role.name }));
  }, [token]);
  ...
  const accessDenied = useEmptyListAccessCheck({
    ...
    error,
    loading,
    resource: listResourceAccess.clusterroles,
    namespace: null,
  });
