...
| `GET /api/view/resources` | Backend-owned static resource/view descriptor bundle for list labels, icons, scope, access-review targets, sidebar grouping, and list view policy such as available quick-filter sources, default sort, filter label, identity fields, and searchable fields. Does not read Kubernetes and is safe for the UI to cache/fallback locally. |
