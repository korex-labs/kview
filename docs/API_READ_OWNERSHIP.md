...
| `GET /api/dashboard/dataplane` | Dataplane-only cached dashboard projection: plane/scope, visibility, coverage, resource totals, cached metrics usage, cache traffic, and scheduler statistics. Signal detection/history/sorting are skipped and signal/derived payloads are omitted. |
