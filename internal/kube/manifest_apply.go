...
func ApplyManifest(ctx context.Context, c *cluster.Clients, defaultNamespace string, manifest string) (applied int, skipped int, err error) {
	...
	for _, doc := range docs {
		...
		if mapErr != nil {
			log.Printf("github.com/korex-labs/kview/v5/apply: RESTMapping failed gvk=%s name=%s ns=%s: %v",
		...
		if marshalErr != nil {
			log.Printf("github.com/korex-labs/kview/v5/apply: marshal failed gvk=%s name=%s ns=%s: %v",
		...
		if applyErr != nil {
			log.Printf("github.com/korex-labs/kview/v5/apply: patch failed gvk=%s name=%s ns=%s: %v",
