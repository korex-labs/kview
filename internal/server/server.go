...
type Server struct {
	...
	podDebugLocks  keyedRequestLocks
}

type keyedRequestLockEntry struct {
	token chan struct{}
	refs  int
}

type keyedRequestLocks struct {
	mu      sync.Mutex
	entries map[string]*keyedRequestLockEntry
}

func (locks *keyedRequestLocks) acquire(ctx context.Context, key string) (func(), error) {
	locks.mu.Lock()
	if locks.entries == nil {
		locks.entries = map[string]*keyedRequestLockEntry{}
	}
	entry := locks.entries[key]
	if entry == nil {
		entry = &keyedRequestLockEntry{token: make(chan struct{}, 1)}
		entry.token <- struct{}{}
...
func (locks *keyedRequestLocks) releaseRef(key string, entry *keyedRequestLockEntry) {
	locks.mu.Lock()
